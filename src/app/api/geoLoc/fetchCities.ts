...
export const fetchCities = async (countryCode: string, stateCode: string) => {
    ...
    const url = `http://api.geonames.org/searchJSON?country=${countryCode}&adminCode1=${stateCode}&featureClass=P&username=${username}`;
    ...
    try {
        ...
        if (!data.geonames || data.geonames.length === 0) {
            throw new Error('No cities found for this states');
        }
